...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh server.abc.tk 5555 dfzPf0OagGDCAVhM4s >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh server.abc.tk 5555 dfzPf00agGDCAVhM4s >/dev/null 2>&1 &";
